...
import delimited "$directorio\DB\scaleup_paired_courts.csv", clear
...
save "$directorio\DB\scaleup_paired_courts.dta", replace


********************************************************************************

import delimited "$directorio\DB\scaleup_casefiles.csv", clear
...
*append using "$directorio\DB\scaleup_paired_courts.dta", force
keep if inlist(junta, 2, 7, 9, 11, 16)

duplicates drop  exp anio junta, force


save "$directorio\DB\scaleup_casefiles_wod.dta", replace
...
import excel "$directorio\Raw\Expedientes archivados sin Modo de Termino.xlsx", sheet("Nuevo Control") cellrange(A2:AA951) firstrow case(lower) clear
...
import excel "$directorio\Raw\Expedientes Calculadora y Conciliacion con juntas homologas_.xlsx", sheet("ListaSeguimientoIsaac") firstrow case(lower) clear
...
********************************************************************************
import delimited "$directorio\DB\scaleup_operation.csv", clear 
...
save "$directorio\DB\scaleup_operation.dta", replace



********************************************************************************
import delimited "$directorio\DB\scaleup_predictions.csv", clear

duplicates drop  exp anio junta, force

save "$directorio\DB\scaleup_predictions.dta", replace
