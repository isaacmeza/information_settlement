...
df_seg <- read_excel('../Raw/scaleup_operation.xlsm', skip = 6) %>% 
...
df_seg %>%
  select(starts_with('conciliador_')) %>% 
...
df_seg <- df_seg %>%
            mutate(notificado = as.numeric(emplazamiento == 1)) %>%
...
# Luego, truncamos probabilidades de parte actora a min 10%
 ...
 df_seg <- df_seg %>%
            mutate_at(vars(contains('cantidad')), as.numeric)  %>%
...
# Guardamos los datos:
write.csv(df_seg, '../DB/scaleup_operation.csv', na = '')
saveRDS(df_seg, '../DB/scaleup_operation.RDS')
