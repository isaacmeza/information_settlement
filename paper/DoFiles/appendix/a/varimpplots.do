import delimited "C:\Users\isaac\Dropbox\Apps\ShareLaTeX\information_lawyer_quality\_aux\df_pre_imp.csv", clear
...
forvalues model = 1/15 {
	...
	*Order 
	...
	twoway (rcap  lo_c_m`model' hi_c_m`model' ft ,  xaxis(1) horizontal color(navy%90) lw( medthick )) ///
		...
		(scatter  ft imp_mean_m`model' , xaxis(2)  msymbol(O) color(red*0.8%80) ), scheme(S2mono) graphregion(color(white)) legend(off) xtitle("Variable importance", axis(2)) xtitle("Linear projection", axis(1)) ytitle("") xline(0, axis(2)) xline(0, axis(1)) 
		
	graph export "C:\Users\isaac\Dropbox\Apps\ShareLaTeX\Information Settlement_2020\paper\Figuras\varimp_m`model'.pdf", replace
	...
	twoway (rcap  lo_c_m`model' hi_c_m`model' ft ,   horizontal color(navy%90) lw( medthick )) ///
			...
			, scheme(S2mono) graphregion(color(white)) legend(off)  xtitle("Linear projection") ytitle("") xline(0) xline(0) 

	graph export "C:\Users\isaac\Dropbox\Apps\ShareLaTeX\Information Settlement_2020\paper\Figuras\coef_m`model'.pdf", replace
			
	twoway 	(rcap  lo_i_m`model' hi_i_m`model' ft ,   horizontal color(red*0.8%80) lw( medthick )) ///
			(scatter  ft imp_mean_m`model' , ylabel(1(1)10, valuelabel angle(0)) msymbol(O) color(red*0.8%80) ), scheme(S2mono) graphregion(color(white)) legend(off) xtitle("Variable importance")  ytitle("") xline(0) xline(0)
				
			
	graph export "C:\Users\isaac\Dropbox\Apps\ShareLaTeX\Information Settlement_2020\paper\Figuras\imp_m`model'.pdf", replace	
	restore
}
