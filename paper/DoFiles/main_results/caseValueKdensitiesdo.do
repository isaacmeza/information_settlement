...
twoway (kdensity liq_total_laudo if treatment==2 & liq_total_laudo<650000 & modoTermino==2 ,  lwidth(medthick) lpattern(solid) color(black)) ||
		...
		legend(lab(1 "Treatment") lab(2 "Control")) xtitle("Calculator Predicted Amounts for Court Win") title("Calculator Predicted Amounts for Court Win") 
...
forvalues i = 1/2{
   ...
   twoway (kdensity liq_total_laudo if treatment==2 & liq_total_laudo<650000 & modoTermino==2  & phase ==`i',  lwidth(medthick) lpattern(solid) color(black)) ||
		...
		legend(lab(1 "Treatment") lab(2 "Control")) xtitle("Calculator predicted settlement") title("Calculator Predicted Amounts for Court Win") 
		subtitle("Unresoved Cases, By treatment, truncated at 99%") ytitle("kdensity") scheme(s2mono) graphregion(color(white)) ylabel(0 (0.000006) 0.000006);
#delimit cr
