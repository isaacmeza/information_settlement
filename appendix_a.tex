...
\begin{table}[!ht]
    ...
    \label{variable_list}
    \begin{center}
        \scriptsize{\input{paper/Tables/Variable_list.tex}}
    \end{center}
     \scriptsize
    \textit{Notes:} 
...
\begin{table}[!ht]
     ...
     \label{ss_survey}
     \begin{center}
     \scriptsize{\input{paper/Tables/surveySS}}
     \end{center}
     \scriptsize
...
\begin{table}[!ht]
    ...
    \begin{subtable}{1\textwidth}
      \centering
        \caption{Phase 1}
        \scriptsize{\input{paper/Tables/Compliance.tex}}
    \end{subtable}%
    ...
    \begin{subtable}{1\textwidth}
      \centering
        \caption{Phase 2}
        \scriptsize{\input{paper/Tables/complianceP2.tex}}
    \end{subtable}
    ...
    \begin{subtable}{1\textwidth}
      \centering
        \caption{\% Shows up}
        \scriptsize{\input{paper/Tables/show_up.tex}}
    \end{subtable}
            \scriptsize
           \\
           \\
...
\begin{table}[H]
\caption{ITT}
\label{stderrrors_itt}
\begin{center}
\scriptsize{\input{paper/Tables/treatment_effect_itt.tex}}
\end{center}
 \scriptsize
...
\begin{table}[!ht]
  \caption{Balance table}
    \label{balance_table}
    \begin{center}
    \scriptsize{\input{paper/Tables/Balance_new.tex}}
    \end{center}
     \scriptsize
    \textit{Notes:} 
    Balance table on basic and strategic variables. The first three columns show phase 1, with means for each variable in the control, calculator, and conciliator treatment groups in the three columns. The next two columns show the control and treatment for phase 2, and finally the last two columns combine both phases. *, **, and *** indicates a difference that is significant at the 10\%, 5\%, and 1\% level, respectively, as compared with the control group. The p-value displayed in the last row corresponds to a test of joint significance.\\
...
We want to estimate the probability that a case with characteristics $X_i$ ends in each of the five ways described above. We have a choice of estimating a single multinomial model or separate bivariate outcome models. We chose the bivariate option for simplicity. But to ensure that the  probabilities summed to one, we set the probability of expiry equal one minus the sum of the probabilities of the other four outcomes.\footnote{We also estimated a multinomial model. The results were similar, which is why we chose the bivariate models for simplicity.} We therefore estimated models for four bivariate outcomes, using each the following methods: (a) Logistic Regression, (b) Probit, (c) Random Forest\footnote{We performed grid search in order to find the best hyperparameter setting. We compared 7 different models with the number of trees ranging between 900 and 1500. Our final model resulted in a Random Forest of 1200 CARTs, which yielded an 86 percent accuracy rate on test classification.}, (d) Single-hidden-layer Neural Network (20 nodes in the hidden layer and 10\% weight decay), (e) Gradient Boosting.\footnote{This was implemented with off the shelf models in R : e1071, randomForest, neuralnet, caTools, mboost.}  Models were estimated in a random sample training set made of 70 percent of the observation. The remaining 30 percent was used as a test set. For each model hyperparameters were chosen to minimize mean square error in the test set (MSE-T) using cross-validation. Once hyperparameters for each model were chosen, we chose among those optimized models bases on their mean square error. For each of the 5 discrete outcomes we want to predict we kept the model with the highest correlation between Y and $\hat{Y}$ in the test set.\footnote{For the outcome of settlement, we find that the correlation between Y and $\hat{Y}$ is very slightly higher for Random Forest than for Logit, but we use the Logit model because it was simpler to implement in the field.} Table \ref{ml_models_comparison_disc} shows the accuracy rate for each of these 5 outcomes and for each of the 5 models, highlighting in grey the one chosen. The correlations range from 0.61 to 0.93 for the models selected. 
 ...
 \begin{subtable}{0.85\textwidth}
     ...
     \raggedleft
     \scriptsize{\input{./paper/Tables/calc_fit_pilot_categorical.tex}}
 \end{subtable}%
 ...
 \begin{subtable}{1\textwidth}
       \centering
         ...
         \scriptsize{\input{./paper/Tables/calc_fit_scaleup_categorical.tex}}
 \end{subtable}
 \\
 \\
 \\
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m1.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Losing court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m2.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Winning court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m3.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Drop}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m4.pdf}
    \end{subfigure}  
    \begin{subfigure}{0.475\textwidth}
        \caption{Winning on court ruling (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m5.pdf}
    \end{subfigure}      
    \end{center}
...
\begin{figure}[H]
    \caption{Variable importance for discrete models}
    \label{PredictorsDiscrete2}
    \begin{center}
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m1.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Losing court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m2.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Winning court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m3.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Drop}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m4.pdf}
    \end{subfigure}  
    \begin{subfigure}{0.475\textwidth}
        \caption{Winning on court ruling (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m5.pdf}
    \end{subfigure}      
    \end{center}
     \scriptsize  This figure shows the main features and the role they play in each of the above models. We estimate the effect of this features on the predictions made by this models : $\log{\widehat{pr_i}} = \alpha + \sum_{j}\beta_j \log{X_{i,j}} + \epsilon$.  Qualitatively, these betas can be used to assess the elasticity of the prediction function with the features, they can be interpreted as the elasticity of the partial effect of variable $X_{j}$ on the model's prediction. By including all the variables we are essentially building a linear approximation around the model's prediction. Note that for the linear models we recover the original point estimates.  We only plot the 20 (out of 72) most important features defined as the ones with the biggest estimates in absolute value.  
          %\footnotesize{ \textit{Do file: }  \texttt{coefplots.do}}
\end{figure}
...
\begin{center}
 ...
 \begin{subtable}{0.85\textwidth}
     ...
     \raggedleft
     \scriptsize{\input{./paper/Tables/calc_fit_pilot_continuous.tex}}
 \end{subtable}%
 ...
 \begin{subtable}{1\textwidth}
       \centering
         ...
         \scriptsize{\input{./paper/Tables/calc_fit_scaleup_continuous.tex}}
 \end{subtable}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m6.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m7.pdf}
    \end{subfigure}
   \begin{subfigure}{0.475\textwidth}
        \caption{Settlement (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m8.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Court ruling (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m9.pdf}
    \end{subfigure}     
    \end{center}
...
\begin{figure}[H]
    \caption{Variable importance for continuous models (total compensation)}
    \label{PredictorsCont1}
    \begin{center}
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m6.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m7.pdf}
    \end{subfigure}
   \begin{subfigure}{0.475\textwidth}
        \caption{Settlement (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m8.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Court ruling (Phase 2 \& 3)}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m9.pdf}
    \end{subfigure}     
    \end{center}
     \scriptsize    This figure shows the main features and the role they play in each of the above models. We estimate the effect of this features on the predictions made by this models : $\log{\widehat{pr_i}} = \alpha + \sum_{j}\beta_j \log{X_{i,j}} + \epsilon$.  Qualitatively, these betas can be used to assess the elasticity of the prediction function with the features, they can be interpreted as the elasticity of the partial effect of variable $X_{j}$ on the model's prediction. By including all the variables we are essentially building a linear approximation around the model's prediction. Note that for the linear models we recover the original point estimates.  We only plot the 20 (out of 72) most important features defined as the ones with the biggest estimates in absolute value.
          %\footnotesize{ \textit{Do file: }  \texttt{coefplots.do}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m10.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Losing court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m11.pdf}
    \end{subfigure}
   \begin{subfigure}{0.475\textwidth}
        \caption{Winning court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m12.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Expiry }
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m13.pdf}
    \end{subfigure}     
        \begin{subfigure}{0.475\textwidth}
        \caption{Drop }
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef_m14.pdf}
    \end{subfigure}     
    \end{center}
     \scriptsize This figure shows the main features and the role they play in each of the above models. We estimate the effect of this features on the predictions made by this models : $\log{\widehat{pr_i}} = \alpha + \beta_j \log{X_{i,j}} + \epsilon$. Note that we are leaving out all but the variable in question. Qualitatively, these betas can be used to assess the elasticity of the prediction function with the features, they can be interpreted as the elasticity of the partial effect of variable $X_{j}$ on the model's prediction.  We only plot the 20 (out of 72) most important features defined as the ones with the biggest estimates in absolute value.  
          %\footnotesize{ \textit{Do file: }  \texttt{coefplots.do}}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.475\textwidth}
        \caption{Settlement}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m10.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Losing court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m11.pdf}
    \end{subfigure}
   \begin{subfigure}{0.475\textwidth}
        \caption{Winning court ruling}
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m12.pdf}
    \end{subfigure}
    \begin{subfigure}{0.475\textwidth}
        \caption{Expiry }
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m13.pdf}
    \end{subfigure}     
        \begin{subfigure}{0.475\textwidth}
        \caption{Drop }
        \centering
        \includegraphics[width=\textwidth]{paper/Figuras/coef1_m14.pdf}
    \end{subfigure}     
    \end{center}
     \scriptsize This figure shows the main features and the role they play in each of the above models. We estimate the effect of this features on the predictions made by this models : $\log{\widehat{pr_i}} = \alpha + \sum_{j}\beta_j \log{X_{i,j}} + \epsilon$.  Qualitatively, these betas can be used to assess the elasticity of the prediction function with the features, they can be interpreted as the elasticity of the partial effect of variable $X_{j}$ on the model's prediction. By including all the variables we are essentially building a linear approximation around the model's prediction. Note that for the linear models we recover the original point estimates.  We only plot the 20 (out of 72) most important features defined as the ones with the biggest estimates in absolute value.
          %\footnotesize{ \textit{Do file: }  \texttt{coefplots.do}}
...
\label{covariate_distribution}
    \begin{center}
        \begin{subfigure}{0.7\textwidth}
            ...
            \centering
            \includegraphics[width=0.7\textwidth]{paper/Figuras/covariates_continous.tiff}
        \end{subfigure}
        \begin{subfigure}{0.7\textwidth}
