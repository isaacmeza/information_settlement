...
\begin{table}[!ht]
    ...
    \begin{center}
        ...
        \scriptsize{\input{./paper/Tables/TableReg1_log.tex}}
    \end{center}
    \scriptsize
%\begin{figurenotes}
...
\begin{table}[!ht]
    ...
    \begin{center}
    \scriptsize{\input{paper/Tables/negative_returners_balance.tex}}
    \end{center}
     \scriptsize
...
%    \begin{center}
%    \scriptsize{\input{paper/Tables/settlementq_time.tex}}
%    \end{center}
%     \scriptsize
%    \textit{Notes:} 
...
\begin{table}[!ht]
    ...
    \label{ep_balance}
    \begin{center}
    \scriptsize{\input{paper/Tables/ep_balance.tex}}
    \end{center}
     \scriptsize
    \textit{Notes:} The table shows results of regressions with the specification $y_i = \alpha_t + \sum_{j = 1,2}\beta_{j} T_{j} + \epsilon_{i}$, where j refers to the treatment assignment ${calculator, conciliator}$. The sample is limited to the cases in which the plaintiff was present at the hearing. All right-hand side variables are standardized so that the constant reflects balance in the control group. Each row represents a different regression with the indicated independent variable. The regressions all include subcourt fixed effects. 
...
\begin{table}[!ht]
    ...
    \label{pactor_reg}
    \begin{center}
    \scriptsize{\input{paper/Tables/pactor_reg.tex}}
    \end{center}
    \scriptsize
...
%     \begin{center}
%      \centering
%        \scriptsize{\input{paper/Tables/NN_match_lr.tex}}
%    \end{center}%
%  \scriptsize
%    \textit{Notes:}
...
\begin{table}[!ht]
    ...
    \label{table_fs_cf}
    \begin{center}
    \scriptsize{\input{paper/Tables/fs_treatment_effects.tex}}
    \end{center}
     \scriptsize
    \textit{Notes:} The first column repeats the regression in column 5 of \ref{Treatment_effects} using a probit specification rather than a linear probability model. The second column is a OLS first stage for employee present included just for comparison with columns 3 and 4. The third column is the first stage of the control function regression shown in column 9 of \ref{Treatment_effects}. Columns 4 and 5 repeat the control function regression using individual dummy variables for each-hour hearing time. Column 4 shows the first stage and column 5 the outcome regression including the residual from the regression in column 4. See the notes to \ref{Treatment_effects} for further details. 
         
    %\textit{Do file: } \texttt{treatment\_effects\_IV\_CF.do}
\end{table}
...
\begin{table}[!ht]
    ...
    \begin{center}
    \scriptsize{\input{paper/Tables/BalanceTime}}
    \end{center}
    \scriptsize
    \textit{Notes:} This table presents how balanced are casifiles with respect to the instrument of Employee Presence, namely that the hearing took place during the first or last hour of the day. The p-value presented in the last row corresponds to a test of joint signifficance. 
...
\begin{table}[!ht]
    ...
    \label{te_end_mode}
    \begin{center}
    \scriptsize{\input{paper/Tables/treatment_effects_end_mode.tex}}
    \end{center}
     \scriptsize
    \textit{Notes:} 
...
\begin{table}[!ht]
    ...
    \begin{center}
    \scriptsize{\input{paper/Tables/te_heterogeneity_1.tex}}
    \end{center}
...
\begin{table}[!ht]
    \caption*{}
    \begin{center}
    \scriptsize{\input{paper/Tables/te_heterogeneity_2.tex}}
    \end{center}
     \scriptsize
    ...
    %\textit{Do file: } \texttt{te\_heterogeneity.do}
\end{table}
%Trick to reset table numbering 
...
\begin{table}[!ht]
    ...
    \begin{center}
    \scriptsize{\input{paper/Tables/te_placebos.tex}}
    \end{center}
     \scriptsize
    ...
    %\textit{Do file: } \texttt{te\_placebo.do}
\end{table}
...
%        \scriptsize{\input{paper/Tables/balance_match.tex}}
%    \end{subtable}%
...
%        \scriptsize{\input{paper/Tables/unconfoundedness_match.tex}}
%    \end{subtable}
...
%    \caption{Comparison of Settlement Amounts}
%    \label{match_table}
%    \scriptsize{\input{./paper/Tables/NN_match.tex}}
%\begin{figurenotes}
...
\label{update_rel_oc}
    ...
    \begin{subtable}{0.9\textwidth}
      \centering
        ...
        \scriptsize{\input{paper/Tables/update_reg_theta_rel_oc.tex}}
    \end{subtable}%
    ...
    \begin{subtable}{0.9\textwidth}
      \centering
        ...
        \scriptsize{\input{paper/Tables/update_reg_theta_rel_uc.tex}}
    \end{subtable} 
     \scriptsize
    \\
    \\
...
\textit{Notes:} The table shows the duration of cases in phases 1 and 2, measured in days from the date of filing to the date of the final resolution. All regressions control for the age of the case at the time of treatment, and include dummies for phase 2 and the subcourt where the case was heard. Column 1 is an OLS regression and Columns 2 and 3 are Cox proportional hazards models, showing the coefficients rather than proportional hazards. Columns 1 and 2 include all cases and Column 3 removes from the sample cases that were settled. The sample in all regressions is limited to cases with private lawyers. The sample is slightly smaller than that in Table \ref{te_bylawyer} because we eliminate a few cases with a missing date of filing. 
\end{table}
...
\begin{figure}[!ht]
 \caption{Stylized Depiction of the Labor Justice Process}
    \begin{center}
        \includegraphics[width=0.9\textwidth]{paper/Figuras/Stylized Diagram of Case Origination.pdf}
        \end{center}
...
\begin{figure}[!ht]
   ...
   \begin{center}
       \scriptsize
        \includegraphics[width=0.5\textwidth]{paper/Figuras/P3Treatments.pdf}
    \end{center}
...
\begin{figure}[!ht]
    ...
    \begin{center}
        \includegraphics[width=0.6\textwidth]{paper/Figuras/kdensityRatio.pdf}
         \end{center}
              \scriptsize
...
\begin{figure}[!ht]
   ...
   \begin{subfigure}{0.45\textwidth}
        \caption{Phase 1}
        \centering
        \includegraphics[width=\linewidth]{paper/Figuras/OutcomesByTreatment_P1.pdf}
    \end{subfigure}
   \begin{subfigure}{0.45\textwidth}
        \caption{Phase 2}
        \centering
        \includegraphics[width=\linewidth]{paper/Figuras/OutcomesByTreatment_P2.pdf}
    \end{subfigure}
...
\begin{figure}[!ht]
   ...
   \begin{subfigure}{0.45\textwidth}
        \caption{Phase 1}
        \centering
        \includegraphics[width=\linewidth]{paper/Figuras/Calculator_CourtWin_Unresolved_P1.pdf}
    \end{subfigure}
   \begin{subfigure}{0.45\textwidth}
        \caption{Phase 2}
        \centering
        \includegraphics[width=\linewidth]{paper/Figuras/Calculator_CourtWin_Unresolved_P2.pdf}
    \end{subfigure}
