...
\begin{table}[!ht]
    ...
    \end{center}
     \scriptsize
    \textit{Notes:}  Column (1) is a `reduced form' OLS regression, and column (2) conditions on employee \emph{not} being present. This serves as a test that time of the hearing does not affect settlement for reasons other than the plaintiff's presence. In the bottom panel we included the p-value for the null that the instrumental variables are equal to zero. The third column is a OLS first stage for employee present, and column (4) is the probit FS used in the control function of table \ref{Treatment_effects}. See the notes to \ref{Treatment_effects} for further details. 
...
\begin{figure}[!ht]
    ...
    \end{subfigure}        
    \end{center}
      \scriptsize \textit{Notes:} Difference in thousand pesos for amounts (panel on left) and percentage points for probabilities (panel on right) comparing the subject expects and what our models predict. Probabilities are the probability the party's side wins the case. Note that for the employee and employee lawyer, the distribution for amounts is much more skewed to the right than for firm lawyers. This is expected, since the former are thinking about expected collections and the latter about expected payments. 
