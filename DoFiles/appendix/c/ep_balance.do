...
replace c_antiguedad = log(c_antiguedad+1)
tab junta, gen(junta)
...
foreach var of varlist `bvc' {	
	...
	if "`var'" == "abogado_pub"{
	outreg2 using ".\paper\Tables\reg_results\ep_balance.xls", replace ctitle("`var'") ///
	addstat(treatment=0, `pval')  keep(2.treatment)  dec(3)
	} 
	else {
	    outreg2 using ".\paper\Tables\reg_results\ep_balance.xls", append ctitle("`var'") ///
