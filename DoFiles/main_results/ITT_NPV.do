...
merge 1:1 junta exp anio using ".\DB\inicialesP1Faltantes_wod.dta", ///
...
ritest altT _b[altT] _b[c.altT#c.p_actor], reps(1000): reg npv_wz c.altT##c.p_actor  `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

reg npv_wz i.treatment i.p_actor i.treatment#i.p_actor `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)	
...
outreg2 using ".\paper\Tables\reg_results\welfareEffectsP12.xls" if !missing(asinhNPVImputed), replace ctitle("asinhNPV") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', calcPVal, `testInteraction', pvalueRI, `pvalNoInteract', pvalueRIInteraction,  `pvalInteract') keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor) dec(0)


*2) IHS NPV (0s)
ritest altT _b[altT] _b[c.altT#c.p_actor], reps(1000): reg asinhNPV c.altT##c.p_actor  `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

reg asinhNPV i.treatment i.p_actor i.treatment#i.p_actor `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)	
	qui test 2.treatment + 2.treatment#1.p_actor = 0
	local testInteraction=`r(p)'
	...
	local DepVarMean=r(mean)
outreg2 using ".\paper\Tables\reg_results\welfareEffectsP12.xls" if !missing(asinhNPVImputed), append ctitle("asinhNPV") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', calcPVal, `testInteraction', pvalueRI, `pvalNoInteract', pvalueRIInteraction,  `pvalInteract') keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor) dec(2)

*3) NPV winsorsized (calculator)
ritest altT _b[altT] _b[c.altT#c.p_actor],  reps(1000): reg npvImputed_wz c.altT##c.p_actor  `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

reg npvImputed_wz i.treatment i.p_actor i.treatment#i.p_actor `controls' , vce(cluster cluster_v)	
	qui test 2.treatment + 2.treatment#1.p_actor = 0
	local testInteraction=`r(p)'
	...
	local DepVarMean=r(mean)
outreg2 using ".\paper\Tables\reg_results\welfareEffectsP12.xls", append ctitle("asinhNPVImputed") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', IntMean, `IntMean', calcPVal, `testInteraction', pvalueRI, `pvalNoInteract', pvalueRIInteraction,  `pvalInteract') keep(2.treatment missingCasefiles 1.p_actor 2.treatment#1.p_actor) dec(0)

*4) IHS NPV winsorsized (calculator)
ritest altT _b[altT] _b[c.altT#c.p_actor], reps(1000): reg asinhNPVImputed c.altT##c.p_actor  `controls'    if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

reg asinhNPVImputed i.treatment i.p_actor i.treatment#i.p_actor `controls'  , vce(cluster cluster_v)
	qui test 2.treatment + 2.treatment#1.p_actor = 0
	local testInteraction=`r(p)'
...
ritest altT _b[altT] _b[c.altT#c.p_actor], reps(1000): reg asinhNPVImputed_robust c.altT##c.p_actor  `controls'   if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

reg asinhNPVImputed_robust i.treatment i.p_actor i.treatment#i.p_actor `controls' , vce(cluster cluster_v)	
	qui test 2.treatment + 2.treatment#1.p_actor = 0
	local testInteraction=`r(p)'
...
ritest altT _b[altT], reps(1000): reg asinhNPV altT `controls' if !missing(asinhNPVImputed), vce(cluster cluster_v)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]

reg asinhNPV i.treatment `controls'  if !missing(asinhNPVImputed), vce(cluster cluster_v)	
	qui su asinhNPV if e(sample) & treatment == 1
	local DepVarMean=r(mean)
outreg2 using ".\paper\Tables\reg_results\welfareEffectsP12.xls" if !missing(asinhNPVImputed), append ctitle("asinhNPV") ///
addtext(Casefile Controls, Yes) addstat(DepVarMean, `DepVarMean', pvalueRI, `pvalNoInteract') keep(2.treatment missingCasefiles) dec(2)
