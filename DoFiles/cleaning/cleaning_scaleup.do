...
import delimited "$scaleup\DB\scaleup_paired_courts.csv", clear
...
save "$scaleup\DB\scaleup_paired_courts.dta", replace


********************************************************************************

import delimited "$scaleup\DB\scaleup_casefiles.csv", clear
...
*append using "$scaleup\DB\scaleup_paired_courts.dta", force
keep if inlist(junta, 2, 7, 9, 11, 16)

duplicates drop  exp anio junta, force


save "$scaleup\DB\scaleup_casefiles_wod.dta", replace
...
import excel "$scaleup\Raw\Expedientes archivados sin Modo de Termino.xlsx", sheet("Nuevo Control") cellrange(A2:AA951) firstrow case(lower) clear
...
import excel "$scaleup\Raw\Expedientes Calculadora y Conciliacion con juntas homologas_.xlsx", sheet("ListaSeguimientoIsaac") firstrow case(lower) clear
...
********************************************************************************
import delimited "$scaleup\DB\scaleup_operation.csv", clear 
...
********************************************************************************
import delimited "$scaleup\DB\scaleup_predictions.csv", clear

duplicates drop  exp anio junta, force

save "$scaleup\DB\scaleup_predictions.dta", replace
