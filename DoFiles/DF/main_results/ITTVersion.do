...
egen fechaJunta = group(junta fecha)
	...
	*Same day conciliation
	...
	ritest altT _b[altT], samplingsourcefile("./_aux/sampleP1.dta") samplingmatchvar(junta exp anio) reps(10000): areg seconcilio altT `controls' if phase==1, robust  cluster(fecha) abs(fecha)
	matrix pvalues=r(p) 
	...
	addtext(Court Dummies, Yes, Casefile Controls, Yes) keep(2.treatment 1.p_actor 2.treatment#1.p_actor ) ///
	...
	*Interaction employee was present
	...
	addtext(Court Dummies, Yes, Casefile Controls, Yes) addstat(Dependent Variable Mean, `DepVarMean', Interaction Mean,`IntMean', test interaction,`testInteraction', pvalueRI, `pvalNoInteract', pvalueRIInteraction,  `pvalInteract') ///
...
restore
	...
	*Same day conciliation
	...
	keep if phase == 2
	//ritest altT _b[altT], reps(10000) seed(125) cluster(fechaJunta) saveresampling("./_aux/sampleP2.dta"): reg seconcilio altT `controls' if phase==2, robust  cluster(fechaJunta)
	ritest altT _b[altT], samplingsourcefile("./_aux/sampleP2.dta") samplingmatchvar(junta exp anio) reps(10000): reg seconcilio altT `controls' if phase==2, robust  cluster(fechaJunta) 
	matrix pvalues=r(p) 
	local pvalNoInteract = pvalues[1,1]

	

	qui reg seconcilio i.treatment `controls' if treatment!=0 & phase==2, robust  cluster(fechaJunta)
	qui su seconcilio if e(sample) & treatment == 1
	local DepVarMean=r(mean)
	outreg2 using  "./Tables/reg_results/treatment_effectsITT.xls", append ctitle("Same day. P2")  ///
	addtext(Court Dummies, Yes, Casefile Controls, Yes) addstat(Dependent Variable Mean, `DepVarMean', pvalueRI, `pvalNoInteract') ///
	keep(2.treatment 1.p_actor 2.treatment#1.p_actor ) dec(3)

	
	*Interaction employee was present
ritest altT _b[altT] _b[c.altT#c.p_actor], samplingsourcefile("./_aux/sampleP2.dta") samplingmatchvar(junta exp anio) reps(10000): reg seconcilio c.altT##c.p_actor  `controls' if phase==2, robust  cluster(fechaJunta)
	matrix pvalues=r(p)
	local pvalNoInteract = pvalues[1,1]
	local pvalInteract = pvalues[1,2]

	qui reg seconcilio i.treatment##i.p_actor `controls' if treatment!=0 & phase==2 , robust  cluster(fechaJunta)
	qui test 2.treatment + 2.treatment#1.p_actor = 0
	local testInteraction=`r(p)'
	qui su seconcilio if e(sample) & treatment == 1 & p_actor == 1
	local IntMean=r(mean)
	...
	*********************************
	...
	save "./_aux/samplePooled2.dta", replace
	restore
	*/
