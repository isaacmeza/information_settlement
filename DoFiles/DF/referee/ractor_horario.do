...
keep seconcilio convenio_2m convenio_5m fecha junta exp anio fecha treatment p_actor abogado_pub numActores time_hr p_ractor
gen phase=2
tempfile p2
save `p2', replace
...
eststo clear 
drop if time_hr8==1
eststo : reg p_ractor  time_hr2-time_hr7 i.anioControl i.numActores i.junta i.phase, r
test  time_hr2 ==  time_hr3 == time_hr4 == time_hr5 == time_hr6== time_hr7==0
estadd scalar Fstat = `r(F)' 
estadd scalar pvalue = `r(p)' 
esttab using "./Tables/reg_results/ractor_horario.csv", se r2 ${star} b(a3) ///
	scalars("Fstat Fstat" "pvalue pvalue")
