...
keep seconcilio convenio_2m convenio_5m fecha junta exp anio fecha treatment p_actor abogado_pub numActores time_hr salario_diario giro_dummy* trabajador_base gen    reinst indem prima_antig prima_vac hextra rec20 prima_dom desc_sem desc_ob sarimssinf utilidades c_antiguedad c_indem c_prima_antig c_rec20 c_ag c_vac c_prima_vac c_desc_sem c_desc_ob   c_total min_indem min_prima_antig min_ag min_vac  min_ley
gen phase=2
tempfile p2
save `p2', replace
...
rename (c_antiguedad c_indemnizacion c_primaantiguedad c_veintedias c_aguinaldo c_vacaciones c_primavacacional c_descansosemanal c_descansoobligatorio) (c_antiguedad c_indem c_prima_antig c_rec20 c_ag c_vac c_prima_vac c_desc_sem c_desc_ob) 

rename (min_indem min_primaantig min_aguinaldo min_vacaciones min_ley) (min_indem min_prima_antig min_ag min_vac  min_ley) 
tab giro, gen(giro_dummy)
drop giro_dummy1
...
tab anioControl, gen(ac_dummy) 
drop ac_dummy1
tab numActores, gen(nac_dummy) 
drop nac_dummy1
/*

keep seconcilio altT   salario_diario trabajador_base gen   reinst indem prima_antig prima_vac hextra rec20 prima_dom desc_sem desc_ob sarimssinf utilidades c_antiguedad c_indem c_prima_antig c_rec20 c_ag c_vac c_prima_vac c_desc_sem c_desc_ob   c_total min_indem min_prima_antig min_ag min_vac  min_ley p_actor ac_dummy* nac_dummy*

duplicates drop

foreach var of varlist seconcilio altT   salario_diario trabajador_base gen   reinst indem prima_antig prima_vac hextra rec20 prima_dom desc_sem desc_ob sarimssinf utilidades c_antiguedad c_indem c_prima_antig c_rec20 c_ag c_vac c_prima_vac c_desc_sem c_desc_ob   c_total min_indem min_prima_antig min_ag min_vac  min_ley p_actor ac_dummy* nac_dummy* {
	di "`var'"
	drop if missing(`var')
}

export delimited using "C:\Users\isaac\Dropbox\ReplicationFinal\_aux\grf.csv", replace
*/
cap drop out
...
preserve
keep if phase==1
keep seconcilio altT anioControl numActores
*Randomization Inference (Fisher)
qui ritest altT _b[altT], reps(10000) seed(9435) : reg seconcilio altT i.anioControl i.numActores
local pval_ri = r(p)[1,1]
restore	
*Robust standard error (Eicker-Huber-White)
qui reg seconcilio altT i.anioControl i.numActores if phase==1, robust
local pval_r=r(table)[4,1]
*Clustered std. errors (Liang-Zeger(1986))
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(cluster cluster_v)
local pval_cl=r(table)[4,1]
*Modify the robust variance calculation (Davidson-MacKinnon(1993)) (Angrist-Pischke(2009))
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(hc2)
local pval_hc2=r(table)[4,1]
*Modify the robust variance calculation (Davidson-MacKinnon(1993)) (Angrist-Pischke(2009))
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(hc3)
local pval_hc3=r(table)[4,1]
*Jackknife
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(jackknife)
local pval_jk=r(table)[4,1]
*Bootstrap (student-t)
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(bootstrap, rep(10000))
local pval_bt=r(table)[4,1]
*Bootstrap (BCa) (Efron (1987), Efron-Tibshirani(1993))
qui reg seconcilio altT d_anioC* d_num* if phase==1, vce(bootstrap, rep(10000) bca)
local pval_bca=r(table)[4,1]
*Small sample CRVE (Pustejovsky-Tipton(2018), Imbens-Kolesar(2016))
eststo : reg_sandwich seconcilio altT d_anioC* d_num* if phase==1, cluster(cluster_v) 
...
preserve
keep if phase==1
keep seconcilio altT p_actor anioControl numActores 
*Randomization Inference (Fisher)
qui ritest altT _b[altT] _b[c.altT#c.p_actor], reps(10000) seed(9435) : reg seconcilio c.altT##c.p_actor i.anioControl i.numActores
local pval_ri = r(p)[1,1]
local pval_ri_int = r(p)[1,2]
restore
*Robust standard error (Eicker-Huber-White)
qui reg seconcilio altT p_actor interactT i.anioControl i.numActores if phase==1, robust
local pval_r=r(table)[4,1]
local pval_r_int=r(table)[4,3]
*Clustered std. errors (Liang-Zeger(1986))
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(cluster cluster_v)
local pval_cl=r(table)[4,1]
local pval_cl_int=r(table)[4,3]
*Modify the robust variance calculation (Davidson-MacKinnon(1993)) (Angrist-Pischke(2009))
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(hc2)
local pval_hc2=r(table)[4,1]
local pval_hc2_int=r(table)[4,3]
*Modify the robust variance calculation (Davidson-MacKinnon(1993)) (Angrist-Pischke(2009))
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(hc3)
local pval_hc3=r(table)[4,1]
local pval_hc3_int=r(table)[4,3]
*Jackknife
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(jackknife)
local pval_jk=r(table)[4,1]
local pval_jk_int=r(table)[4,3]
*Bootstrap (student-t)
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(bootstrap, rep(10000))
local pval_bt=r(table)[4,1]
local pval_bt_int=r(table)[4,3]
*Bootstrap (BCa) (Efron (1987), Efron-Tibshirani(1993))
qui reg seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, vce(bootstrap, rep(10000) bca)
local pval_bca=r(table)[4,1]
local pval_bca_int=r(table)[4,3]
*Small sample CRVE (Pustejovsky-Tipton(2018), Imbens-Kolesar(2016))
eststo : reg_sandwich seconcilio altT p_actor interactT d_anioC* d_num* if phase==1, cluster(cluster_v) 
local pval_pt=2*ttail(e(dfs)[1,1], abs(e(b)[1,1]/sqrt(e(V)[1,1])))
...
preserve
*Probit (FS)
qui probit p_actor i.altT time_instrument i.anioControl i.numActores i.junta i.phase
cap drop xb
predict xb, xb
...
foreach var of varlist altT* {
	...
	qui probit p_actor `var' time_instrument i.anioControl i.numActores i.junta i.phase
	cap drop xb
	qui predict xb, xb
	*Generalized residuals
...
preserve
keep doble_convenio altT fecha_alta
*Randomization Inference (Fisher)
qui ritest altT _b[altT], cluster(fecha_alta) reps(10000) seed(9435) : reg doble_convenio altT
local pval_ri = r(p)[1,1]
restore
*Robust standard error (Eicker-Huber-White)
reg doble_convenio altT, robust 
local pval_r=r(table)[4,1]
*Clustered std. errors (Liang-Zeger(1986))
qui reg doble_convenio altT, vce(cluster fecha_alta)
local pval_cl=r(table)[4,1]
*Jackknife
qui reg doble_convenio altT, vce(jackknife, cluster(fecha_alta))
local pval_jk=r(table)[4,1]
*Bootstrap (student-t)
qui reg doble_convenio altT, vce(bootstrap, cluster(fecha_alta) rep(10000))
local pval_bt=r(table)[4,1]
*Bootstrap (BCa) (Efron (1987), Efron-Tibshirani(1993))
qui reg doble_convenio altT, vce(bootstrap, cluster(fecha_alta) rep(10000) bca)
local pval_bca=r(table)[4,1]
*Moulton (1986)
qui moulton doble_convenio altT, cluster(fecha_alta) 
local pval_moulton=r(table)[4,1]
*Bias-reduced linearization ("clustered-HC2") (Bell-McCaffrey(2002))
qui brl doble_convenio altT, cluster(fecha_alta)
local pval_brl=r(table)[4,1]
*Small sample CRVE (Pustejovsky-Tipton(2018), Imbens-Kolesar(2016))
eststo : reg_sandwich doble_convenio altT, cluster(fecha_alta)
local pval_pt=2*ttail(e(dfs)[1,1], abs(e(b)[1,1]/sqrt(e(V)[1,1])))
